#!/bin/bash

# Get the latest version from Git tag
version=$(git describe --abbrev=0 --tags)

directory="releases/$version"
if [ ! -d "$directory" ]; then
    mkdir "$directory"
    echo "Directory created: $directory"
else
    echo "Directory already exists: $directory"
fi

cp -r src/mule-app-analyser/resources $directory/resources

env GOPATH=$PWD/src GOOS=linux GOARCH=amd64 go build -o=$directory/mule-app-analyser-$version-linux src/mule-app-analyser/main.go
env GOPATH=$PWD/src GOOS=darwin GOARCH=amd64 go build -o=$directory/mule-app-analyser-$version-osx-amd64 src/mule-app-analyser/main.go
env GOPATH=$PWD/src GOOS=darwin GOARCH=arm64 go build -o=$directory/mule-app-analyser-$version-osx-arm64 src/mule-app-analyser/main.go
env GOPATH=$PWD/src GOOS=windows GOARCH=amd64 go build -o=$directory/mule-app-analyser-$version-win.exe src/mule-app-analyser/main.go